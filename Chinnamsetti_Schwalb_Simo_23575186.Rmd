...
editor_options: 
  markdown: 
    wrap: sentence
---
#TODO: SUPPRESS WARNING MESSAGES IN OUTPUT!!! 
...
- IF NEEDED UNCOMMENT BELOW: All rows are removed where the counted number of horses for the event is less than indicated by column HORSE_COUNT
...
Keep the following columns based on domain knowledge and careful inspection of the dataset e.g. owner, dam and damsire are removed as there are too many different entries, but jockey, trainer and sire are kept as there are only a few. BSP (Betfair Starting Price) for WIN and PLACE are removed, as Imp_Perc is the inverse of the odds (it is simply calculated s 1/BSP) and easier to interpret:
...
-   **PLACE**: The result indicating whether the horse placed in the top positions. - replaced by Imp_Perc_PLACE
...
Draw has missing values as this feature is only relevant for flat races. We need to keep it though, as it is believed to influence the performance.
Check datatypes. 
```{r}
str(df.horse)
```
Some variable types should be modified further for better analysis. 
...
THIS IS FOR BCKUP: Change rows where pos is not a number in a new data frame df.horses (the reason is that if required we can build a ML model that predict the number of non finishers):
```{r}
...
```
About 20% of horses have no official rating and 2% have no rp rating. Plot rpr vs or
```{r}
...
```
It seems like rpr is a better predictor than or, however there are a few instances with missing values in rpr that have values in or.We will use Linear Regression Imputation as it is straightforward and likely accurate given the correlation between rpr and or. However, if other variables also strongly influence rpr, we will consider the multivariable regression approach.

```{r}
cor(df.horse$rpr_num, df.horse$Imp_Perc_PLACE, use = "complete.obs")
cor(df.horse$rpr_num, df.horse$Imp_Perc_WIN, use = "complete.obs")
```
Hence, for now Linear Regression Imputation will be used:

```{r}
# Step 0: Check the initial missing rpr values
...
summary(df.horse$rpr_num)
```
It can be seen that missing values reduced from 1201 to 438.
Now remove columns or, or_num, rpr and rows where rpr_num is NA
```{r}
# Remove columns or, or_num, and rpr
df.horse <- df.horse %>%
  select(-or, -or_num, -rpr)

# Remove rows where rpr_num is NA
df.horse <- df.horse %>%
...
```
Lets create another feature, previous_finish which looks at the previous date the horse ran, then divide that pos column by ran column and multiply that result by 10. This is hence a normalised previous position. If a horse had no previus races at Lingfield, this value will take the ean of 5.1.
...
df.horse <- df.horse %>%
  ...
  ungroup() %>%  # Remove grouping
  select(-previous_pos, -previous_ran) %>%  # Remove columns
  mutate(previous_finish = ifelse(is.na(previous_finish), 5.1, previous_finish))  # Replace NA with 5.1

# View the resulting data frame
head(df.horse)
```
-LETS PLOT ALL PREDICTOR VARIABLES (EDA) VS RESPONSE VARIABLE AVG_SPEED AND INVESTIGATE
  -Purpose:
    -Understand how each predictor influences the response variable (avg_speed). 
    -Identify linear/non-linear relationships, outliers, or any        clusters in the data.
  -Steps:
    -Plot continuous variables (e.g., dist_m, lbs) using scatter plots.Plot categorical variables (e.g., type, going) using boxplots or violin plots.
    -Use correlation matrices to quantify relationships between predictors and avg_speed.
    -Here Log-transforms can also be checked
  -Expected Outcome:
    -Insights into which predictors strongly influence avg_speed.
    -Guidance on feature selection for downstream modeling.


-LETS CHECK IF WE CAN DEVELOP MODEL THAT CAN PREDICT PLACE AS BINARY
-LETS CHECK IF WE CAN DEVELOP MODEL THAT CAN PREDICT PLACE AS REGRESSION - RESPONSE VARIABLE Imp_Perc_PLACE
-INVESTIGATE IF WE CAN BUILD A FEASIBLE MODEL THAT PREDICTS THE NUMBER OF NON-FINISHERS PER DAY
...
The updated distribution looks more balanced than before! While there is still some imbalance (e.g., G is much more frequent than Other or C). 
Ensure that training data is representative by using stratified sampling. This prevents the model from being biased toward majority classes during training:

```{r}
## Stratified train-test split
trainIndex <- createDataPartition(df.horse$sex_4, p = 0.85, list = F)
...
df.horse %>%
 ...
 geom_point()
```
Somewhat more blue and purple dots are visible in the upper right corner, and somewhat more green and red in the bottom left. 
But there is definitely no clear-cut distinction or identifyable clusters. 
...
Create variables to access data: 
```{r}
train<-df.horse%>%
slice(indices)
test_in<-df.horse%>%
...
Visualization is not an option since we have 20+ variables thus we cannot visualize in 20+ dimensions. 
Plot the above 2 dimensions instead: 
```{r}
plot(horse_svm, train, log_secs ~ log_lbs)
```
